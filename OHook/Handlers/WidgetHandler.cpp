...
void WidgetHandler::Tick(const UObject* Context, UFunction* Function, void* Parms) {
    ...
    if (CurrentViewModel == EViewModel::PremiumStore) {
        ...
        if (CurrentStateWidget && CurrentStateWidget->IsA(UWBP_PremiumStoreScreen_V2_CM_C::StaticClass())) {
            ParseStoreContent(reinterpret_cast<UWBP_PremiumStoreScreen_V2_CM_C*>(CurrentStateWidget));
        }
    }
}
...
void WidgetHandler::ParseStoreContent(UWBP_PremiumStoreScreen_V2_CM_C* StoreScreen) {
    if (!StoreScreen) return;
    if (!KnownGoodGuids.empty()) return;
    for (auto WidgetModel : StoreScreen->Cached_Storefront_Response.Models) {
        for (auto Variant : WidgetModel.Variants) {
            for (auto ItemId : Variant.ItemIds) {
                KnownGoodGuids.push_back(ItemId);
...
EWidgetType WidgetHandler::ParseWidget(std::string WidgetName) {
    ...
    else if (WidgetName.starts_with("WBP_VillagerStore_CM_C")) {
        return EWidgetType::VillagerStore;
    }
    else if (WidgetName.starts_with("WBP_PlayerStorageModal_CM_C")) {
        return EWidgetType::PlayerStorage;
    }
    else {
